...
BOT_TOKEN = "7540635983:AAGCt7e0qeMejscfOjcmGSSYz9HwrqsFsrA"
...
def download_and_process_doc():
    ...
    excel_folder = "excel"
    main_schedule = 'schedule.xlsx'  # Файл в корне проекта
    ...
    try:
        ...
        doc_filename = f'schedule_{current_date}.doc'
        ...
        try:
            ...
            if found_table:
                # Сохраняем в папку excel с датой
                excel_filename = f'schedule_{current_date.strftime("%Y%m%d")}.xlsx'
                excel_path = os.path.join(excel_folder, excel_filename)
                wb.save(excel_path)

                # Сохраняем копию в корень проекта
                try:
                    # Если существует старый файл, удаляем его
                    if os.path.exists(main_schedule):
                        os.remove(main_schedule)

                    # Сохраняем новую копию в корень проекта
                    wb.save(main_schedule)
                    logging.info(f"Successfully created Excel files: {excel_path} and {main_schedule}")

                    return main_schedule
                except Exception as e:
                    logging.error(f"Error copying to main schedule file: {e}")
                    return excel_path
            else:
                logging.error(f"Table for {target_header} not found")
                return None
...
# Добавьте эту функцию в основной код и вызывайте её периодически
def update_schedule():
...
def schedule_updates():
    try:
        ...
        # Планируем обновление расписания
        schedule.every().day.at("00:01").do(update_schedule)
        logging.info("Scheduled daily update at 00:01")
...
def send_shift_notifications_for_time(target_time):
    ...
    if current_time != target_time:
        ...
        return  # Этот return находится внутри функции, что правильно
    ...
    for login, data in user_data.items():
        try:
            ...
            if not shifts:
                logging.warning(f"No shifts found")
                continue
...
def get_user_info(login):
    schedule_data = load_schedule_from_excel()
    for entry in schedule_data:
        if entry["id"] == user_data[login]["user_id"]:
            tomorrow = datetime.now().date() + timedelta(days=1)
            shifts = []
            for i in range(3):
                check_date = tomorrow + timedelta(days=i)
                index = check_date.day - 1
                if index < len(entry["shifts"]):
                    shifts.append((check_date, entry["shifts"][index]))
                else:
                    shifts.append((check_date, None))
            return shifts, entry.get("name")
    return None, None
...
def calculate_worked_time(login):
    try:
        workbook = openpyxl.load_workbook(EXCEL_FILE)
        sheet = workbook.active
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=4, values_only=True):
            ...
            if login in user_data:
                user_id_dict = user_data[login]
                telegram_id = user_id_dict["user_id"]
        ...
        logging.error(f"Error loading schedule from Excel: {e}")
        return []
...
def check_file_command(message):
    ...
    if user_id in ADMIN_USERS:
        try:
            current_date = datetime.now().strftime("%Y%m%d")
            doc_path = os.path.join("documents", f'schedule_{current_date}.docx')
            ...
            if os.path.exists(doc_path):
                ...
                bot.reply_to(message,
                             ...
                             f"Начало файла (hex): {content.hex()[:50]}...")
            else:
                bot.reply_to(message, "Файл не найден")
        except Exception as e:
            bot.reply_to(message, f"Ошибка при проверке файла: {str(e)}")
    else:
        bot.reply_to(message, "У вас нет прав для выполнения этой команды")
